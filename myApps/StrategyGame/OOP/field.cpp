...
int Field::checkEmptySpace(int code){
    ...
    if (in > 29){
        throw LogicException("checkEmptySpace", "No space for units");
    }
    return in;
}
...
void Field::setUnit1(Unit* unit){
    if (unit->x < this->rows && unit->y < this->columns && this->countOfObjects < this->maxNumOfObjects){
        int in;
        try {
            in = checkEmptySpace(unit->id.code);
        } catch (LogicException l) {
            cout << l.what() << endl;
            cout << "You summon too many units\n";
        }
...
Unit* Field::changeUnit1 (Unit* unit){
    try {
        for (int i = 0; i < 6; i++){
            for (int j = 0; j < 30; j++){
                if (units[i][j] && units[i][j] != unit){
                    if (!isalpha(units[i][j]->sym)){
                        throw LogicException("changeUnit1", 0);
                    }
                    return units[i][j];
                }
            }
...
Unit* Field::changeUnit2(Unit* unit){
        for (int i = 6; i < 12; i++){
            for (int j = 0; j < 30; j++){
                if (units[i][j] && units[i][j] != unit){
                    if (!isalpha(units[i][j]->sym)){
                        throw LogicException("changeUnit2", 0);
                    }
                    return units[i][j];
                }
            }
...
void Field::move(Unit* unit, int x, int y, TerminalLog* termLog, FileLog* fileLog, int logFlag, int ruleFlag){                        // передвижение юнитов
        ...
        if (ruleFlag == 1){
            if (y == 17/2 && x == 0 && unit->id.code > 5){
                field[y][x]->base->hp -= unit->force;
                if (field[y][x]->base->hp <= 0){
                    field[y][x]->base = nullptr;
                }
                return;
            }
            if (y == 17/2 && x == 17-1 && unit->id.code < 6){
                field[y][x]->base->hp -= unit->force;
                if (field[y][x]->base->hp <= 0){
                    field[y][x]->base = nullptr;
                }
        ...
        if (unit->id.code < 6 && field[y][x]->unit && field[y][x]->unit->id.code > 5){
            ...
            if (field[y][x]->unit->hp <= 0){
                if (field[23/2][0]->base && ruleFlag == 2){
                    field[23/2][0]->base->gold += field[y][x]->unit->cost/2;
                }
        ...
        if (unit->id.code > 5 && field[y][x]->unit && field[y][x]->unit->id.code < 6){
            ...
            if (field[y][x]->unit->hp <= 0){
                if (field[23/2][23-1]->base && ruleFlag == 2){
                    field[23/2][23-1]->base->gold += field[y][x]->unit->cost/2;
                }
...
int Field::isRich(){
    if (getBaseGold(0, rows/2) == 1000){
        return 1;
    }
    if (getBaseGold(columns-1, rows/2) == 1000){
        return 2;
    }
    return 0;
}
...
int Field::isDestroyed(){
    if (!field[rows/2][0]->base){
        return 1;
    }
    if (!field[rows/2][columns-1]->base){
        return 2;
    }
    return 0;
}

void Field::summonGold(){
    int x = rand()%23;
    int y = rand()%23;
    while(field[y][x]->base || field[y][x]->unit || field[y][x]->nObject){
        int x = rand()%23;
        int y = rand()%23;
    }
    field[y][x]->nObject = new Gold;
}
...
void Field::quickLoad(Memento* snap){
    if (snap == nullptr){
        throw LogicException("quickLoad", 0);
    }
        for (int i = 0; i < this->rows; i++){
            for (int j = 0; j < this->columns; j++){
                if (this->field[i][j]->base)
...
void MoveUpCommand::execute(Unit* unit, TerminalLog* termLog, FileLog* fileLog, int logFlag, int ruleFlag){
    gField->move(unit, unit->x, unit->y-1, termLog, fileLog, logFlag, ruleFlag);
}

void MoveDownCommand::execute(Unit* unit, TerminalLog* termLog, FileLog* fileLog, int logFlag, int ruleFlag){
    gField->move(unit, unit->x, unit->y+1, termLog, fileLog, logFlag, ruleFlag);
}

void MoveLeftCommand::execute(Unit* unit, TerminalLog* termLog, FileLog* fileLog, int logFlag, int ruleFlag){
    gField->move(unit, unit->x-1, unit->y, termLog, fileLog, logFlag, ruleFlag);
}

void MoveRightCommand::execute(Unit* unit, TerminalLog* termLog, FileLog* fileLog, int logFlag, int ruleFlag){
    gField->move(unit, unit->x+1, unit->y, termLog, fileLog, logFlag, ruleFlag);
}
            ...
            for (int j = 0; j < columns; j++){
                ...
                *loadFile >> str;
                if (str == "skip"){
                    ...
                    if (!isalpha(str[0])){
                        throw TypeInputException("string");
                    }
                    if (str == "g"){
                        this->field[i][j]->nObject = new Gold;
                    }
